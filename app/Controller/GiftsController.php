...
class GiftsController extends AppController {
    ...
    public function compose($steamid) {
        ...
        if ($user_id == $recipient_id) {
            $this->Session->setFlash('You cannot send a gift to yourself.', 'flash_closable', array('class' => 'error'));
            $this->redirect(array('controller' => 'Items', 'action' => 'index'));
            return;
        }
    ...
    public function package($steamid) {
        ...
        foreach ($giftDetails as $key => $detail) {
            ...
            if (empty($userItems[$item_id]) || $userItems[$item_id] < $quantity) {
                $this->Session->setFlash("There are no longer enough {$items[$item_id]['plural']} in your inventory to package this gift.", 'flash_closable', array('class' => 'error'));
    ...
    public function send($steamid) {
        ...
        if (empty($gift)) {
            $this->Session->setFlash('Oops! It appears you have not prepared a gift for this recipient.', 'flash_closable', array('class' => 'error'));
            $this->redirect(array('action' => 'compose', 'id' => $steamid));
            return;
        }
        ...
        if (!$result['Gift'] || in_array(false, $result['GiftDetail'])) {
            $this->Session->setFlash('There was an error sending the gift. Please contact an administrator', 'flash_closable', array('class' => 'error'));
            $this->UserItem->query('UNLOCK TABLES');
            $this->User->saveField('locked', 0);
            $this->redirect(array('action' => 'compose', 'id' => $steamid));
            return;
        ...
        $this->Session->setFlash("Your gift has been sent! Gift number - #{$this->Gift->id}", 'flash_closable', array('class' => 'success'));
        $this->redirect(array('action' => 'compose', 'id' => $steamid));
    }
}